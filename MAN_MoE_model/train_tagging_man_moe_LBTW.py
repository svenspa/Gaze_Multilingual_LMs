                    ...
                    for lang in opt.all_langs:
                        for i in range(0,numLoss):
                            ...
                            if i==0:
                                ...
                                if i==0:
                                    init_loss = [l_d.item(), l_d1.item()]
                                loss_ratio = [x/y for x,y in zip([l_d.item(), l_d1.item()], init_loss)]
                                class_weights = [abs(x)**opt.alpha for x in loss_ratio]
                                class_weights = [x / sum(class_weights)*2 for x in class_weights]
                                l_d = class_weights[0]*l_d + class_weights[1]*l_d1
                        wandb.log({f"loss D {lang}": l_d.item()})
                        l_d.backward()
                    ...
                    for i in range(0,numLoss):
                        ...
                        if j==0:
                            ...
                            if i==0:
                                init_loss = [l_d.item(), l_d1.item()]
                            loss_ratio = [x/y for x,y in zip([l_d.item(), l_d1.item()], init_loss)]
                            class_weights = [x**opt.alpha for x in loss_ratio]
                            class_weights = [x / sum(class_weights)*2 for x in class_weights]
                            l_d = class_weights[0]*l_d + class_weights[1]*l_d1
