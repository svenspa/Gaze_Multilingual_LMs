...
class LanguageDiscriminatorMT(LanguageDiscriminator):
    def __init__(self,
                model,
                mlp_layers,
                input_size,
                ...
                batch_norm=False,
                model_args=None):
        super().__init__(model, mlp_layers, input_size, hidden_size,num_langs,
                        dropout, batch_norm, model_args)
        self.num_gaze_feat = num_gaze_feat
        self.obj11 = nn.Linear(hidden_size, num_langs)
        self.obj12 = nn.LogSoftmax(dim=-1)
        self.obj21 = nn.Linear(hidden_size, num_gaze_feat)

    def forward(self, input):
        data, lengths = input
        if self.pool:
            data = self.pool((data, lengths))
        self.MTnet = nn.Sequential(*(list(self.net.children())[:-2]))
        out = self.MTnet(data)
        out_obj11 = self.obj11(out)
        out_obj12 = self.obj12(out_obj11)
        out_obj21 = self.obj21(out)
        if opt.loss.lower() == 'l2':
            out_obj12 = functional.relu(out_obj12)
            out_obj12 /= torch.sum(out_obj12, dim=1, keepdim=True)
        return out_obj12, out_obj21
