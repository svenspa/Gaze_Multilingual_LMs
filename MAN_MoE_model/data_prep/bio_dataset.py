...
def get_ner_datasets(vocab, char_vocab, tag_vocab, data_dir, lang):
    print(f'Loading NER data for {lang} Language..')
    train_set = NERDataset(os.path.join(data_dir, f'{lang}.train'),
                           vocab, char_vocab, tag_vocab, update_vocab=True, remove_empty=opt.remove_empty_samples)
    dev_set = NERDataset(os.path.join(data_dir, f'{lang}.dev'),
                           vocab, char_vocab, tag_vocab, update_vocab=True)
    test_set = NERDataset(os.path.join(data_dir, f'{lang}.test'),
                           vocab, char_vocab, tag_vocab, update_vocab=True)
    return train_set, dev_set, test_set, train_set


def get_train_on_translation_ner_datasets(vocab, char_vocab, tag_vocab, data_dir, lang):
    print(f'Loading Train-on-Translation NER data for {lang} Language..')
    train_set = NERDataset(os.path.join(data_dir, f'eng2{lang}_{lang}.train'),
                           vocab, char_vocab, tag_vocab, update_vocab=True)
    dev_set = NERDataset(os.path.join(data_dir, f'{lang}.dev'),
                           vocab, char_vocab, tag_vocab, update_vocab=True)
    test_set = NERDataset(os.path.join(data_dir, f'{lang}.test'),
...
def get_test_on_translation_ner_datasets(vocab, char_vocab, tag_vocab, data_dir, lang):
    print(f'Loading Test-on-Translation NER data for {lang} Language..')
    train_set = NERDataset(os.path.join(data_dir, f'eng.train'),
                           vocab, char_vocab, tag_vocab, update_vocab=True)
    dev_set = NERDataset(os.path.join(data_dir, f'eng2{lang}_eng.dev'),
                           vocab, char_vocab, tag_vocab, update_vocab=True)
    test_set = NERDataset(os.path.join(data_dir, f'eng2{lang}_eng.test'),
                           vocab, char_vocab, tag_vocab, update_vocab=False)
    return train_set, dev_set, test_set, train_set
