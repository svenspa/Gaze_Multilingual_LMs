...
def load_gaze(lang, n_gaze_feat, EMB_DIR):
    input_file = f'{EMB_DIR}gaze/{lang}.meco'
    gaze_features = {}
    with io.open(input_file,'r', encoding='utf-8') as inf:
        next(inf)
        for line in inf:
            line = line.split('\t')
            gaze_features[str(line[1])] = [float(line[x]) for x in range(3,3 + n_gaze_feat)]
