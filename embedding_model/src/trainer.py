...
class Trainer(object):
    ...
    def __init__(self, embs, mappings, discriminators, gaze, params):
        """
        ...
        self.init_loss = 0
        self.init_lossM = 0
        self.loss_multi = params.loss_multi
        self.loss_add = params.loss_add
    ...
    def get_dis_xy(self, lang1, lang2, volatile):
        ...
        src_ids = torch.LongTensor(bs).random_(len(self.vocabs[lang1]) if mf == 0 else mf) #choose bs random indices
        tgt_ids = torch.LongTensor(bs).random_(len(self.vocabs[lang2]) if mf == 0 else mf) #choose bs random indices
    ...
    def get_dis_xy_gaze(self, lang1, lang2, volatile):
        """
        Get discriminator input batch / output target.
        Encode from lang1, decode to lang2 and then discriminate
        """
        # select random word IDs
        bs = self.params.batch_size
        mf = min(len(self.gaze[lang1]), len(self.gaze[lang2]))
        #assert mf <= min(map(len, self.vocabs.values()))
        src_words = random.choices(list(self.gaze[lang1].keys()), k=bs) #choose bs random indices
        tgt_words = random.choices(list(self.gaze[lang2].keys()), k=bs) #choose bs random indices
        src_ids = torch.tensor([self.vocabs[lang1].word2id[str(i)] for i in src_words])
        tgt_ids = torch.tensor([self.vocabs[lang2].word2id[str(i)] for i in tgt_words])
        src_ids = src_ids.to(self.params.device)
        tgt_ids = tgt_ids.to(self.params.device)
        ...
        # input / target
        x = torch.cat([tgt_emb, tgt_emb], 0)
        y = torch.FloatTensor(2 * bs).zero_()

        src_y = torch.tensor([self.gaze[lang1][w] for w in src_words])
        tgt_y = torch.tensor([self.gaze[lang2][w] for w in tgt_words])
        y = torch.cat([tgt_y, tgt_y], 0)
    ...
    def dis_step(self, stats, method):
        """
        ...
        for lang2 in self.params.all_langs:
            ...
            preds, _ = self.discriminators[lang2](x.detach())
            loss_m += F.binary_cross_entropy(preds, y)
            #wandb.log({f"loss_m_{lang2}": loss_m})
            if self.params.use_mt_learning:
                x, y = self.get_dis_xy_gaze(lang1, lang2, volatile=True)
                preds_m, preds_a  = self.discriminators[lang2](x.detach())
                loss_a += self.loss_multi*F.l1_loss(preds_a.view(-1),y.view(-1)) 
                #wandb.log({f"loss_a_{lang2}": loss_a})
        if method == 'rlw':
            weight = F.softmax(torch.randn(2), dim=-1)
        elif method == 'lin':
            weight[0] = (1-weight[0])+ self.nEpoch*(1/(self.params.n_epochs/2))
            weight[1] = 1-weight[0]
        elif method == 'lbtw':
            if self.nIter==0 and self.nDisStep ==0:
                self.init_loss = [loss_m.item(), loss_a.item()]
            loss_ratio = [x/y for x,y in zip([loss_m.item(), loss_a.item()], self.init_loss)]
            class_weights = [abs(x)**self.params.alpha for x in loss_ratio]
            weight = [x / sum(class_weights)*2 for x in class_weights]
        loss_t = weight[0]*loss_m + weight[1]*(loss_a + self.loss_add)
        #wandb.log({f"loss_t": loss_t})

        # check NaN
        if (loss_m != loss_m).any():
            logger.error("NaN detected (discriminator)")
            exit()
            stats['DIS_COSTS'].append(loss_t.item())
    ...
    def mapping_step(self, stats, method):
        """
        ...
        for lang1 in self.params.all_langs:
            ...
            preds, _ = self.discriminators[lang2](x)
            loss_m += F.binary_cross_entropy(preds, 1 - y)
        loss_m = self.params.dis_lambda * loss_m
            ...
            for j, lang2 in enumerate(self.params.all_langs):
                if lang1 == lang2:
                    ...
                    # use one of the provided dictionary
                        ...
                        DIC_EVAL_PATH = os.path.join(self.params.EMB_DIR, 'crosslingual', 'dictionaries')
    ...
    def mpsr_step(self, stats):
        ...
        stats['MPSR_COSTS'].append(loss_m.item())
        # optim
    ...
    def reload_best(self):
        ...
        tgt_lang = self.params.tgt_lang
        for src_lang in self.params.src_langs:
            path = os.path.join(self.params.loc_path,
                                f'best_mapping_{src_lang}2{tgt_lang}.t7')
