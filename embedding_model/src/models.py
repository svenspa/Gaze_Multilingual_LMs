...
from .utils import load_embeddings, normalize_embeddings, load_gaze
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.layers(x).view(-1), 0

class DiscriminatorMT(Discriminator):

    def __init__(self, params, lang):
        super().__init__(params, lang)

        self.n_gaze_feat = params.n_gaze_feat
        self.obj11 = nn.Linear(self.dis_hid_dim, 1)
        self.obj12 = nn.Sigmoid()
        self.obj2 = nn.Linear(self.dis_hid_dim, self.n_gaze_feat)

    def forward(self, x):
        assert x.dim() == 2 and x.size(1) == self.emb_dim
        self.MTnet = nn.Sequential(*(list(self.layers.children())[:-2]))
        out = self.MTnet(x)
        out_obj11 = self.obj11(out)
        out_obj12 = self.obj12(out_obj11)
        out_obj2 = self.obj2(out)
        return out_obj12.view(-1), out_obj2
...
def build_model(params, with_dis):
    ...
    """
    # source embeddings
    params.vocabs, _src_embs, embs, gaze = {}, {}, {}, {}
    ...
    # discriminators
    if params.use_mt_learning:
         discriminators = {lang: DiscriminatorMT(params, lang)
    ...
    # Gaze
    for lang in params.all_langs:
        gaze[lang] = load_gaze(lang, params.n_gaze_feat, params.EMB_DIR)

    return embs, mappings, discriminators, gaze
