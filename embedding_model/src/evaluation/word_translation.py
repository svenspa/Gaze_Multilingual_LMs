...
def get_word_translation_accuracy(lang1, word2id1, emb1, lang2, word2id2, emb2, method, dico_eval, params):
    """
    Given source and target word embeddings, and a dictionary,
    evaluate the translation accuracy using the precision@k.
    """
    DIC_EVAL_PATH = os.path.join(params.EMB_DIR, 'crosslingual', 'dictionaries')
    if dico_eval == 'default':
        if params.split is not 'all':
            path = os.path.join(DIC_EVAL_PATH, '%s_%s-%s.5000-6500.txt' % (params.split_o, lang1, lang2))
        else:
            path = os.path.join(DIC_EVAL_PATH, '%s-%s.5000-6500.txt' % (lang1, lang2))
    elif dico_eval == 'train':
        if params.split is not 'all':
            path = os.path.join(DIC_EVAL_PATH, '%s_1000_%s-%s.txt' % (params.split_o, lang1, lang2))
        else:
            path = os.path.join(DIC_EVAL_PATH, '1000_%s-%s.txt' % (lang1, lang2))
    elif dico_eval == 'all':
    ...
    results = []
    top_matches = scores.topk(5000, 1, True)[1]
    for k in [1, 5, 10, 50, 100, 500, 1000, 5000]:
        ...
        match_list = [torch.tensor(i).to(params.device) if not torch.is_tensor(i) else i for i in match_list]
        precision_at_k = 100 * torch.stack(match_list).cpu().numpy().mean()
        ...
        if dico_eval=='train':
            wandb.log({f'val/{params.split_o}_{method}_{lang1}_{lang2} - precision at {k}':precision_at_k})
        if dico_eval=='default':
            wandb.log({f'test/{params.split_o}_{method}_{lang1}_{lang2} - precision at {k}':precision_at_k})
    return results
