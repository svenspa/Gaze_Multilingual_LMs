...
def get_wordsim_scores(language, word2id, embeddings,emb_dir, lower=True):
    """
    Return monolingual word similarity scores.
    """
    dirpath = os.path.join(emb_dir,'monolingual', language)
...
def get_crosslingual_wordsim_scores(lang1, word2id1, embeddings1,
                                    ...
                                    emb_dir, validset, lower=True, ignore_oov=True
                                    ):
    """
    Return cross-lingual word similarity scores.
    """
    if validset=='all':
        f1 = os.path.join(emb_dir,'crosslingual','wordsim', '%s-%s-SEMEVAL17.txt' % (lang1, lang2))
        f2 = os.path.join(emb_dir,'crosslingual','wordsim', '%s-%s-SEMEVAL17.txt' % (lang2, lang1))
    else:
        f1 = os.path.join(emb_dir,'crosslingual','wordsim', '%s-%s-SEMEVAL17-%s.txt' % (lang1, lang2, validset))
        f2 = os.path.join(emb_dir,'crosslingual','wordsim', '%s-%s-SEMEVAL17-%s.txt' % (lang2, lang1, validset))
